...
function getVersion (original) {
    return original.replace(/-[a-zA-Z0-9]+/gi,"")
}

function getHistoryBadges (badgeList) {
    var finalBadgeList = [];
    var upBadge = "<i style=\"color:green\" class=\"fas fa-circle\"></i>";
    var downBadge = "<i style=\"color:red\" class=\"fas fa-circle\"></i>";
    badgeList.forEach((mem)=>{
        (mem) ?    finalBadgeList.push(upBadge) : finalBadgeList.push(downBadge);
    });
    return finalBadgeList.join("")
}
            ...
            success: function (data) {
                if (data.result) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.txs) {
                                data = processTxData(data);
                                sessionStorage.setItem('searchTransaction', JSON.stringify(data.result[0]));
                                window.location.href = transactionExplorer.replace('{id}', text);
                            } else if (data.missed_tx) {
...
function processTxData (data) {
    ...
    for(var i=0; i<data.txs.length; i++){
        temp = {}
        for(var key of Object.keys(data.txs[i])){
            if(!excl.includes(key)){
                temp[key] = data.txs[i][key]
                temp.json = JSON.parse(data.txs[i].as_json)
            }
        }
        res.result.push(temp)
    }
    return res
}

function wrongSearchAlert() {
  var toastHTML = 'Wrong search query! Please enter block height or hash, transaction ID or a Xolentum address!';
  M.toast({html: toastHTML});
}
